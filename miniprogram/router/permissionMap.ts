...
import { canAccess, type FeatureKey } from '../utils/acl';
...
export function canAccessPath(role: UserRole, path: string): { ok: true } | { ok: false; redirectTo: string } {
  const rr = ROUTE_RULES[path];
  if (!rr) return { ok: true };
