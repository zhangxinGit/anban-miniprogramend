...
/** 弹窗触发来源 */
export type AssessmentTriggerSource =
  | 'launch'      // 启动即触发
  | 'login_retry' // 登录完成后重试
  ;

/** 弹窗状态快照 */
export type AssessmentPopupState = {
  active: boolean;
  source: AssessmentTriggerSource;
  ts: number;
};
...
export function resetSessionClosed(): void {
  try {
...
function setLastPopupTime(ts: number): void {
  try {
...
export function shouldShowPopupForLoggedInUser(): boolean {
  // 豁免 1：已完成测评（永久）
  if (isAssessmentCompleted()) return false;
  // 豁免 2：本次会话已关闭
...
/** 标记“需要展示评估弹窗” */
export function markAssessmentPopup(source: AssessmentTriggerSource): void {
  try {
    const state: AssessmentPopupState = { active: true, source, ts: Date.now() };
...
/** 读取并消费弹窗信号（一次性，读完即清除） */
export function consumeAssessmentPopup(): AssessmentPopupState | null {
  try {
    const raw: unknown = wx.getStorageSync(STORAGE_KEYS.pending);
    if (!raw || typeof raw !== 'object') return null;
    const state = raw as Partial<AssessmentPopupState>;
    // 10 分钟内有效
    if (Date.now() - (state.ts || 0) > 10 * 60_000) {
      wx.removeStorageSync(STORAGE_KEYS.pending);
      return null;
    }
    if (state.active !== true) return null;
    wx.removeStorageSync(STORAGE_KEYS.pending);
    return state as AssessmentPopupState;
  } catch {
    return null;
  }
}
...
/** 手动清除弹窗信号 */
export function clearAssessmentPopup(): void {
  try {
    wx.removeStorageSync(STORAGE_KEYS.pending);
  } catch {
    // ignore
 ...
 * 同一时刻只允许一次同步请求（去重）
 */
export async function syncAssessmentCompletedFromServer(): Promise<boolean> {
  ...
  _syncPromise = (async () => {
    try {
      ...
      const resp = await request<{ assessed: boolean }>({
      ...
      if (resp.ok && resp.data) {
        ...
        if (serverAssessed) {
          ...
          try { wx.removeStorageSync(STORAGE_KEYS.completed); } catch {}
        }
