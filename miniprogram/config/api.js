 ...
 */
export function normalizeAppApiPath(path) {
    // 已经是版本化路径
    ...
    if (m1)
        return `/api/${API_VERSION}/${m1[1]}`;
    // /app/api/... → /api/v1/app/...（历史 Nginx 反向路径）
    const m2 = /^\/app\/api\/(.*)$/.exec(path);
    if (m2)
        return `/api/${API_VERSION}/app/${m2[1]}`;
    // 其他 /api/... 路径
    const m3 = /^\/api\/(.*)$/.exec(path);
    if (m3)
        return `/api/${API_VERSION}/${m3[1]}`;
