...
export type AppointmentUpsertInput = Pick<Appointment, 'name' | 'phone' | 'familyAddress' | 'doorplate' | 'date' | 'time' | 'remark'> & {
  serviceName?: string;
...
type BackendAppointment = {
  ...
  reserve_time_range: string | null;
  remark: string | null;
  status: AppointmentStatus;
...
type ParsedFamilyAddress = {
  familyAddress: string;
  detailAddressPrefix: string;
};

function parseFamilyAddress(raw: string | null | undefined): ParsedFamilyAddress {
...
function mergeDetailAddress(prefix: string, doorplate: string | null | undefined): string {
  const prefixValue = String(prefix || '').trim();
  const detailValue = String(doorplate || '').trim();
  if (!prefixValue) return detailValue;
  if (!detailValue) return prefixValue;
  if (detailValue.startsWith(prefixValue)) return detailValue;
...
export function getLastContact(): { name: string; phone: string; familyAddress: string; doorplate: string } {
  try {
    const v = wx.getStorageSync(STORAGE_KEYS.lastContact);
    if (v && typeof v === 'object') {
      const name = typeof (v as any).name === 'string' ? (v as any).name : '';
      const phone = typeof (v as any).phone === 'string' ? (v as any).phone : '';
      const familyAddress = typeof (v as any).familyAddress === 'string' ? (v as any).familyAddress : '';
      const doorplate = typeof (v as any).doorplate === 'string' ? (v as any).doorplate : '';
...
function setLastContact(name: string, phone: string, familyAddress: string, doorplate: string) {
  try {
...
function getGuestAppointment(): Appointment | null {
  try {
...
function setGuestAppointment(appointment: Appointment | null) {
  try {
    if (!appointment) {
...
export async function getAppointment(role: UserRole): Promise<Appointment | null> {
  void role;
  const st = getAuthState();
  if (!isLoggedIn()) return getGuestAppointment();
  if (!st.userId) return null;
  const resp = await request<BackendAppointment | null>({
    url: '/api/app/appointments/current',
    method: 'GET',
  });
  if (!resp.ok) throw new Error(resp.message || '加载预约失败');
  if (!resp.data) return null;
  return mapBackendAppointment(resp.data);
}

export async function submitAppointment(role: UserRole, input: AppointmentUpsertInput): Promise<Appointment> {
  void role;
  const payload = {
    name: input.name,
    phone: input.phone,
  ...
  if (!isLoggedIn()) {
    const resp = await request<BackendAppointment>({
      url: '/api/app/appointments/guest-upsert',
      method: 'POST',
      data: payload,
    });
    if (!resp.ok) throw new Error(resp.message || '提交预约失败');
...
export async function cancelAppointment(role: UserRole): Promise<void> {
  void role;
  if (!isLoggedIn()) {
    ...
    if (Number.isFinite(leadId) && leadId > 0) {
      ...
      if (!resp.ok) throw new Error(resp.message || '取消预约失败');
    }
    setGuestAppointment({
      ...current,
      status: 'CANCELLED',
