...
type BackendBannerRow = JsonRecord;

export type MiniBannerScene = 'HOME' | 'SERVICE_MARKET';
...
function readList(value: unknown): BackendBannerRow[] {
...
export async function getHomeBanners(scene: MiniBannerScene = 'HOME'): Promise<HomeBannerCard[]> {
  const sceneParam = scene === 'SERVICE_MARKET' ? 'SERVICE_MARKET' : 'HOME';
  const result = await request<BackendBannerRow[]>({
