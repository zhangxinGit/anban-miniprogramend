...
export async function loginWithWeChat(payload: WechatLoginPayload): Promise<void> {
  ...
  const wxLogin = await new Promise<{ code: string }>((resolve, reject) => {
    wx.login({
      success: (res) => {
        ...
        else reject(new Error('微信登录失败，请重试 [stage:wx.login]'));
      },
  ...
  if (!resp.ok) throw new Error(`${resp.message || '微信登录失败'} [stage:auth.request] [trace:${traceId}]`);
