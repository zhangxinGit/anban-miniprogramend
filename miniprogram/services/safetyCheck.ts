...
export type SafetyAnswer = { qid: string; value: number };

export type SafetyRespondentProfile = {
...
export type LeadStatus = 'NEW' | 'FOLLOWING' | 'APPOINTED' | 'DEAL';

export type RiskLevelKey = 'LOW' | 'MEDIUM' | 'HIGH';

export function computeScore(answers: SafetyAnswer[]): number {
  return answers.reduce((sum, a) => sum + (a.value | 0), 0);
}

export function computeRiskLevel(score: number): RiskLevelKey {
  if (score <= 2) return 'LOW';
  if (score <= 5) return 'MEDIUM';
  return 'HIGH';
}

export function rebindSafetyRecordsToPhone(_phone: string): void {
...
type LocalRecord = SafetyRecord;
type LocalMap = Record<string, LocalRecord[]>;

function localGetMap(): LocalMap {
  try {
    const v = wx.getStorageSync(LOCAL_KEYS.recordsByKey);
    if (v && typeof v === 'object') return v as LocalMap;
  } catch {}
  return {};
}

function localSetMap(m: LocalMap) {
  try {
...
function localKey(): string {
  ...
  if (isLoggedIn() && st.boundPhone) return `phone:${st.boundPhone.trim()}`;
  if (st.clientId) return `client:${st.clientId.trim()}`;
  return `anon:${st.leadId || 'x'}`;
}

export async function submitSafetyCheck(answers: SafetyAnswer[], profile: SafetyRespondentProfile): Promise<{
  role: UserRole;
  leadId: string;
  userId: string;
  record: SafetyRecord;
  ...
  const score = computeScore(answers);
  const resp = await request<{ assessment_id: number; risk_level: string; lead_id: number; score: number }>({
  ...
  // 统一角色：未成交用户始终为 VISITOR（不再存在 LEAD）
  const nextRole = USER_ROLES.VISITOR as UserRole;
  roleStore.setRole(nextRole);

  const riskMap: Record<string, RiskLevelKey> = {
    low: 'LOW',
    medium: 'MEDIUM',
    high: 'HIGH',
  };
  const record: SafetyRecord = {
    ...
    riskLevel:
      riskMap[String(resp.data.risk_level || '').toLowerCase()] ||
  ...
  if (leadId && st.leadId !== leadId) setLeadId(leadId);

  return { role: nextRole, leadId, userId, record };
}

export async function getSafetyHistory(limit = 3): Promise<{ leadId: string | null; history: SafetyRecord[] }> {
  if (isDevOfflineFallback()) {
    const k = localKey();
    const map = localGetMap();
  ...
  const resp = await request<
    Array<{ id: number; risk_level: string; score: number; answers_json: string; created_at: string }>
  >({
    url: '/api/app/safety/history',
    method: 'GET',
  });
  if (!resp.ok) return { leadId: null, history: [] };
  ...
  const list = (resp.data || []).slice(0, limit).map((x) => {
    let answers: SafetyAnswer[] = [];
    try {
      const v = JSON.parse(x.answers_json || '[]');
      if (Array.isArray(v)) answers = v as SafetyAnswer[];
    } catch {}
    return {
      id: String(x.id),
...
export async function getLeadStatus(): Promise<{ leadId: string | null; status: LeadStatus | null }> {
  const { leadId, role } = getAuthState();
  if (!leadId) return { leadId: null, status: role === USER_ROLES.VISITOR ? null : 'NEW' };
  // 生产：线索状态由后端 lead.status 驱动；当前先维持最小闭环
  return { leadId, status: 'NEW' };
}

export function leadStatusLabel(status: LeadStatus | null): string {
  if (!status) return '未生成线索';
  switch (status) {
    case 'NEW':
      return '已生成线索（待跟进）';
    case 'FOLLOWING':
