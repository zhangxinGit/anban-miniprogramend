export type DutyAlarmToneClass = 'critical' | 'warn' | 'info';
export type DutyAlarmHandlingClass = 'pending' | 'handled';

export type DutyAlarmMeta = {
  recordTypeText: string;
  recordTypeClass: DutyAlarmToneClass;
  handlingText: string;
  handlingClass: DutyAlarmHandlingClass;
};

export type ServiceBookingStatusCode = 'PENDING' | 'COMPLETED' | 'CANCELLED';
export type ServiceBookingStatusClass = 'pending' | 'completed' | 'cancelled';

export type ServiceBookingStatusMeta = {
  status: ServiceBookingStatusCode;
  statusText: string;
  statusClass: ServiceBookingStatusClass;
  canCancel: boolean;
};
...
function isHelpRecord(deviceId: string, title: string): boolean {
  if (deviceId === 'GLOBAL') {
    return true;
  }
...
export function getDutyAlarmMeta(input: {
  ...
  recordTypeClass?: DutyAlarmToneClass | null;
  handledLabel?: string | null;
  handledClass?: DutyAlarmHandlingClass | null;
}): DutyAlarmMeta {
  ...
  const handlingClass: DutyAlarmHandlingClass = handled ? 'handled' : 'pending';
...
export function getServiceBookingStatusMeta(rawStatus: string | null | undefined): ServiceBookingStatusMeta {
