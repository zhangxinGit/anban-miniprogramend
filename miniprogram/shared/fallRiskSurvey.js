/** 判断是否为智能风险题（绑定了产品标签） */
export function isSmartQuestion(q) {
    return !!q.productTag;
}
export const FALL_RISK_SURVEY = [
    {
        key: 'body',
        title: '1. 个人身体状况',
        questions: [
...
export function countFallRiskQuestions() {
    return FALL_RISK_SURVEY.reduce((total, section) => total + section.questions.length, 0);
}
export function computeFallRiskLevel(score) {
    if (score <= 15)
        return 'low';
    if (score <= 30)
        return 'medium';
...
export function fallRiskLevelMeta(level) {
    switch (level) {
        case 'high':
            return {
                label: '高风险',
