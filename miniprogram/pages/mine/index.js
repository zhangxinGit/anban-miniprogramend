...
import { USER_ROLES, isManagementRole, managementRoleLabel } from '../../shared/roles';
...
import { applyPendingReferral, bindReferralCode, clearPendingReferralCode, fetchReferralMine, getPendingReferralCode, normalizeReferralCode, } from '../../services/referral';
import { devSkipLogin } from '../../services/devSkipLogin';
...
import { getFamilyProfile, isFamilyAdmin } from '../../services/familyProfile';
...
import { consumeAssessmentPopup, clearAssessmentPopup, shouldShowPopupForLoggedInUser, recordPopupShown, markSessionClosed, syncAssessmentCompletedFromServer, } from '../../utils/assessmentPopup';
...
function syncTabBarSelection(page) {
    var _a;
    const tabBar = (_a = page.getTabBar) === null || _a === void 0 ? void 0 : _a.call(page);
    if (!isRecord(tabBar)) {
        return;
    }
...
async function getWechatProfile() {
    return await new Promise((resolve) => {
        ...
        wx.getUserProfile({
            desc: '用于完善头像与昵称',
            success: (res) => {
                const userInfo = isRecord(res.userInfo) ? res.userInfo : {};
                resolve({
...
function isUserDeniedPhoneAuth(errMsg) {
    const msg = (errMsg || '').toLowerCase();
    return msg.includes('user deny') || msg.includes('user cancel') || msg.includes('auth deny');
}
function isPhoneAuthPermissionError(errMsg) {
    const msg = (errMsg || '').toLowerCase();
    return msg.includes('operatewxdata:fail') || msg.includes('jsapi has no permission');
}
function normalizePhoneAuthError(errMsg) {
    const msg = (errMsg || '').trim();
    if (!msg)
        return '未获取到手机号授权结果';
    if (isUserDeniedPhoneAuth(msg))
        return '你已取消手机号授权';
    if (isPhoneAuthPermissionError(msg))
        return '当前小程序未开通微信手机号权限';
    return msg.replace(/^getphonenumber:/i, '').trim() || '手机号授权失败';
}
function resolveDisplayMineName(name) {
...
function buildFamilyMemberView(member, index, mePhone, avatarUrl, nickName) {
...
function extractStageLabel(message) {
    ...
    const stage = ((match === null || match === void 0 ? void 0 : match[1]) || '').trim().toLowerCase();
    if (!stage)
        return '未识别';
    if (stage === 'phone.auth')
        return '手机号授权凭证';
...
function stripDebugMarkers(message) {
...
Page({
    ...
    async bindReferral(inviteCode, bindSource) {
        const page = this;
    ...
    scheduleInviteExpiryRefresh() {
        this.clearInviteTimer();
        if (!this.data.familyInviteReady)
            return;
        const ts = Date.parse(this.data.familyInviteExpiresAt || '');
        if (!Number.isFinite(ts))
            return;
    ...
    async onChooseAvatar(e) {
        var _a;
        const tempAvatarUrl = (_a = e === null || e === void 0 ? void 0 : e.detail) === null || _a === void 0 ? void 0 : _a.avatarUrl;
        if (!tempAvatarUrl)
            return;
        try {
    ...
    onAvatarError(e) {
        var _a;
        console.warn('[Mine] 头像图片加载失败! src=', this.data.avatarUrl, 'err=', ((_a = e.detail) === null || _a === void 0 ? void 0 : _a.errMsg) || e.detail);
    ...
    async onGetPhoneNumber(e) {
        ...
        try {
            const errMsg = typeof ((_a = e === null || e === void 0 ? void 0 : e.detail) === null || _a === void 0 ? void 0 : _a.errMsg) === 'string' ? e.detail.errMsg : '';
            const phoneCode = typeof ((_b = e === null || e === void 0 ? void 0 : e.detail) === null || _b === void 0 ? void 0 : _b.code) === 'string' ? e.detail.code.trim() : '';
            const encryptedData = typeof ((_c = e === null || e === void 0 ? void 0 : e.detail) === null || _c === void 0 ? void 0 : _c.encryptedData) === 'string' ? e.detail.encryptedData.trim() : '';
            const iv = typeof ((_d = e === null || e === void 0 ? void 0 : e.detail) === null || _d === void 0 ? void 0 : _d.iv) === 'string' ? e.detail.iv.trim() : '';
    ...
    onReferralCodeInput(e) {
        var _a;
        const value = normalizeReferralCode(((_a = e === null || e === void 0 ? void 0 : e.detail) === null || _a === void 0 ? void 0 : _a.value) || '');
    ...
    onScanBindReferral() {
        ...
        wx.scanCode({
            ...
            fail: (error) => {
                const message = typeof (error === null || error === void 0 ? void 0 : error.errMsg) === 'string' ? error.errMsg : '';
    ...
    async onDevSwitchMode(e) {
        var _a, _b;
        const mode = String(((_b = (_a = e === null || e === void 0 ? void 0 : e.currentTarget) === null || _a === void 0 ? void 0 : _a.dataset) === null || _b === void 0 ? void 0 : _b.mode) || '');
        if (!mode)
            return;
        if (this.data.loginLoading)
            return;
        ...
        try {
            ...
            const targetRole = mode === 'DEAL_NO_DEVICE' ? USER_ROLES.LEAD : USER_ROLES.VISITOR;
    ...
    checkAssessmentPopupAfterLogin() {
        ...
        syncAssessmentCompletedFromServer().then(() => {
            if (!shouldShowPopupForLoggedInUser())
                return;
            recordPopupShown();
    ...
    async reloadSuitableOrders() {
        ...
        try {
            const result = await offlineFirst('ab_offline_suitable_my_orders', () => getMyOrders(), {
                ttlMs: 5 * 60000,
            ...
            safeSetData(page, { suitableOrders, suitableOrdersLoading: false });
            if (result.fresh) {
                cacheForOffline('ab_offline_suitable_my_orders', orders, 5 * 60000);
            }
        }
        catch (error) {
            if (!page._pageHidden) {
