...
Page({
  ...
  async onSendCode() {
    if (this.data.codeSeconds > 0) return;
    // 立即启动倒计时，按钮立即置灰不可点击
    ...
    try {
      ...
      // 发送失败：重置倒计时，恢复按钮
      this.clearCountdown();
      wx.showToast({ title: typeof e?.message === 'string' ? e.message : '发送失败', icon: 'none' });
    }
  },
  ...
  async onLogin() {
    ...
    try {
      ...
      try {
        ...
        const message = typeof e?.message === 'string' ? e.message : '推荐关系绑定失败';
      ...
      wx.showToast({ title: typeof e?.message === 'string' ? e.message : '登录失败', icon: 'none' });
    } finally {
