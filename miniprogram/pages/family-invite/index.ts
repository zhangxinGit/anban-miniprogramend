...
import {
  ...
  previewFamilyInvite,
  setPendingFamilyInviteToken,
} from '../../services/familyInvitation';
...
type InviteErrorView = {
  title: string;
  detail: string;
};

function resolveInviteErrorView(message: string): InviteErrorView {
...
Page({
  ...
  onLoad(query: Record<string, string | undefined>) {
    const token = decodeURIComponent(String(query?.token || '')).trim();
    if (!token) {
      clearPendingFamilyInviteToken();
  ...
  async reload() {
    ...
    try {
      ...
      const message = typeof e?.message === 'string' ? e.message : '邀请链接无效';
  ...
  async tryAccept() {
    if (!this.data.token || this.data.acting || this.data.accepted) return;
    if (!canAutoAccept()) {
    ...
    try {
      ...
      const message = typeof e?.message === 'string' ? e.message : '加入失败';
      const view = resolveInviteErrorView(message);
