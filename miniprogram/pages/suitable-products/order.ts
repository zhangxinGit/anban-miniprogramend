...
import { submitOrder, type SubmitOrderData } from '../../services/suitableProducts';
import { markPageDead, markPageAlive, safeSetData } from '../../utils/pageGuard';
import type { CartItem } from '../../store/cartStore';

type OrderPageCustom = {
  _pageHidden?: boolean;
};
...
Page({
  ...
  onUnload() {
    markPageDead(this as OrderPageCustom);
  },
  ...
  onShow() {
    markPageAlive(this as OrderPageCustom);
    this.loadCartItems();
  },
  ...
  loadCartItems() {
    ...
    const page = this as unknown as OrderPageCustom;
    safeSetData(page, {
      items,
  ...
  onOrderNameInput(e: WechatMiniprogram.Input) {
  ...
  onReceiverAddressInput(e: WechatMiniprogram.Input) {
    this.setData({ receiverAddress: e.detail.value });
  },

  onDeliveryTimeChange(e: WechatMiniprogram.PickerChange) {
    this.setData({ deliveryTime: e.detail.value as string });
  },
  ...
  validateForm(): string | null {
    ...
    if (!orderName.trim()) return '请填写订单名称';
    if (!receiverName.trim()) return '请填写收货人';
    if (!receiverPhone.trim()) return '请填写联系电话';
    if (!/^1\d{10}$/.test(receiverPhone.trim())) return '请输入正确的手机号码';
    if (!receiverAddress.trim()) return '请填写详细收货地址';
    if (!deliveryTime) return '请选择预约配送时间';
    return null;
  },
