...
import { bindDeviceByCode, getDeviceList, recordEmergencyHelp } from '../../services/deviceCenter';
import { getAlarmSubscribeConfig, getAlarmSubscribeState, requestAlarmSubscription, sendAlarmSubscribeTest } from '../../services/alarmSubscription';
...
function formatDeviceNoShort(sn) {
    const normalized = String(sn || '').trim();
    return normalized ? normalized.slice(-4) : '--';
}
function resolveInstallLocation(device) {
    const location = String(device.installLocation || '').trim();
    return location || '未设置';
}
function resolveSignalClass(device) {
    var _a, _b;
    if (device.online !== 'ONLINE')
        return 'offline';
    if ((((_a = device.battery) === null || _a === void 0 ? void 0 : _a.percent) || 0) >= 66)
        return 'strong';
    if ((((_b = device.battery) === null || _b === void 0 ? void 0 : _b.percent) || 0) >= 33)
        return 'medium';
    return 'weak';
}
function formatBatteryBadge(percent) {
    return `${Math.max(0, Math.min(100, percent | 0))}%`;
}
function resolveAlarmSubscribeCard(role, state, configEnabled, configError = '') {
...
Page({
    ...
    onLoad() {
        ...
        this.__unsub = roleStore.subscribe((role) => {
            ...
            if (page._pageHidden)
                return;
            (_a = page.setData) === null || _a === void 0 ? void 0 : _a.call(page, { role });
            this.refreshVisible();
            if (page._reloading)
                return;
            page._reloading = true;
            this.reload().finally(() => { this._reloading = false; });
    ...
    async onShow() {
        ...
        markPageAlive(this);
        const tab = (_a = this.getTabBar) === null || _a === void 0 ? void 0 : _a.call(this);
    ...
    async reload() {
        ...
        try {
            ...
            if (page._pageHidden)
                return;
            const mode = list.length === 0 ? 'EMPTY_BIND' : 'LIST';
    ...
    async refreshAlarmSubscribeCard() {
        ...
        try {
            ...
            if (page._pageHidden)
                return;
            this.applyAlarmSubscribeCard(resolveAlarmSubscribeCard(role, stored, true, getErrorMessage(error, '')));
        }
    },
    applyAlarmSubscribeCard(card) {
        this.setData({
    ...
    onBindCodeInput(event) {
        var _a;
        this.setData({ bindCode: String(((_a = event === null || event === void 0 ? void 0 : event.detail) === null || _a === void 0 ? void 0 : _a.value) || '') });
    },
    onClearBindCode() {
        if (this.data.binding)
            return;
    ...
    async onConfirmBind() {
        ...
        try {
            ...
            wx.hideLoading();
            wx.showToast({
                title: (subscribeResult === null || subscribeResult === void 0 ? void 0 : subscribeResult.status) === 'accepted' ? '绑定成功，已开启提醒' : '绑定成功',
                icon: 'success',
            });
        }
        catch (error) {
    ...
    async onRequestAlarmSubscribe() {
        if (this.data.alarmSubscribeLoading || this.data.alarmSubscribeButtonDisabled)
            return;
    ...
    async onSendAlarmSubscribeTest() {
        if (this.data.alarmSubscribeTestLoading || !this.data.alarmSubscribeTestVisible)
            return;
    ...
    onOpenDevice(e) {
        var _a, _b;
        const id = (_b = (_a = e === null || e === void 0 ? void 0 : e.currentTarget) === null || _a === void 0 ? void 0 : _a.dataset) === null || _b === void 0 ? void 0 : _b.id;
        if (!id)
            return;
