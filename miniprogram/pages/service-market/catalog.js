...
function pruneSnapshot() {
    if (serviceSnapshot.size <= SNAPSHOT_MAX_SIZE)
        return;
    // 删除最早的条目（Map 保持插入顺序）
...
export async function resolveServiceCatalogItemCoverSource(item) {
...
export async function resolveServiceCatalogItemCoverSources(items) {
    return await Promise.all(items.map((item) => resolveServiceCatalogItemCoverSource(item)));
}
export function rememberServiceItems(items) {
    items.forEach((item) => {
        serviceSnapshot.set(item.id, item);
    });
    pruneSnapshot();
}
export function readServiceItem(id) {
    return serviceSnapshot.get(id) || null;
}
export async function fetchServiceCatalogPage(page, size) {
    var _a, _b, _c;
    ...
    if (!resp.ok) {
        const err = new Error(resp.message || '加载服务失败');
        err.code = resp.code;
        throw err;
    }
    const total = Number(((_a = resp.data) === null || _a === void 0 ? void 0 : _a.total) || 0);
    const list = Array.isArray((_b = resp.data) === null || _b === void 0 ? void 0 : _b.list) ? (_c = resp.data) === null || _c === void 0 ? void 0 : _c.list.map(normalizeRow) : [];
    return { total: Number.isFinite(total) ? total : 0, list };
}
export function mergeCatalogItems(current, incoming) {
    const next = [...current];
...
export function filterCatalogItems(items, keyword, moduleTitle) {
...
export function groupCatalogByModule(items) {
